      ...
      <main className="max-w-7xl mx-auto px-4 py-10">
        <section className="text-center mb-10">
          ...
          <p className={`mt-3 max-w-2xl mx-auto ${dm.sub}`}>
            Browse curated  clothes across every genre.
          </p>
        </section>
        ...
        {loading ? (
          <div className="flex flex-col gap-4">
            {[1, 2, 3].map((n) => (
              <div key={n} className={`rounded-2xl border p-6 animate-pulse h-32 ${dm.card}`} />
            ))}
          </div>
        ) : filteredItems.length === 0 ? (
          <div className={`text-center py-24 text-lg ${dm.empty}`}>
            <i className="ti ti-shirt text-4xl mb-3 block opacity-40" />  

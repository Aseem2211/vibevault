      ...
      <nav className="bg-white dark:bg-[#161b35] border-b border-[#E0E4FF] dark:border-[#2a3060] sticky top-0 z-50 shadow-sm transition-colors duration-300">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <Link to="/cart" className="text-[#1A2556] dark:text-[#e8ecff] hover:text-[#FF6B6B] transition">
            ...
            {isLoggedIn && (
              <div className="relative">
                <button
                  ...
                  onBlur={() => { blurTimer.current = setTimeout(() => setProfileOpen(false), 150); }}
            ...
            {isLoggedIn ? (
              <button onClick={handleLogout} className="px-3 sm:px-4 py-2 bg-[#1A2556] dark:bg-[#FF6B6B] text-white text-sm font-semibold rounded-full hover:bg-[#FF6B6B] dark:hover:bg-[#e85555] transition">
                Logout
              </button>
            ) : (
              <Link to="/login" className="px-3 sm:px-4 py-2 bg-[#1A2556] dark:bg-[#FF6B6B] text-white text-sm font-semibold rounded-full hover:bg-[#FF6B6B] dark:hover:bg-[#e85555] transition">
                Login
              </Link>
            )}
            ...
            <button
              ...
              className="sm:hidden text-[#1A2556] dark:text-[#e8ecff] hover:text-[#FF6B6B] transition ml-1"
        ...
        {menuOpen && (
          <div className="sm:hidden border-t border-[#E0E4FF] dark:border-[#2a3060] bg-white dark:bg-[#161b35] px-4 py-3 flex flex-col gap-2">
            <Link to="/sell"
              onClick={() => setMenuOpen(false)}
              className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-[#E0E4FF] dark:bg-[#2a3060] text-[#1A2556] dark:text-[#c0caff] text-sm font-semibold hover:bg-[#d7dbff] dark:hover:bg-[#343d75] transition">
              <i className="fas fa-tag w-4" /> Sell an Item
            </Link>
            <Link to="/rent"
              onClick={() => setMenuOpen(false)}
              className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-[#E0E4FF] dark:bg-[#2a3060] text-[#1A2556] dark:text-[#c0caff] text-sm font-semibold hover:bg-[#d7dbff] dark:hover:bg-[#343d75] transition">
              <i className="fas fa-rotate-left w-4" /> Rent an Item
            </Link>
          </div>
        )}
      </nav>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
          {CATEGORIES.map(cat => (
            ...
            >
              <div className="w-full h-32 sm:h-48 overflow-hidden bg-[#F8F6FF] dark:bg-[#1e2444] group-hover:bg-[#FFE5E5] dark:group-hover:bg-[#3d1a1a] transition">
                {cat.image ? (
              ...
              <div className="p-3 sm:p-5">
                <h3 className="font-bold text-[#1A2556] dark:text-[#e8ecff] text-sm sm:text-base">{cat.label}</h3>
                <p className="text-xs sm:text-sm text-[#4F5B81] dark:text-[#8892be] mt-1 hidden sm:block">{cat.desc}</p>
              </div>
            </Link>
          ))}
        </div>
